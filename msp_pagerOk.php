	...
	*/
	...
	class Multisite_Posts_Core {
		...
		function display_msp_posts( $one_msp_posts, $echo = false, $options = false ) {
			...
			foreach ($z['posts'] as $key => $val){
				$arr_post[$key] = (array) $val;
			} 	
			
			$per_page = $this->options["per_page"];
			$pager = (int) $_GET['pager']; 
			//	if($pager == $pager_pages)
			//	$per_page = ($pager == $pager_pages) && (count($arr_post) % $pager == 0) ? $per_page : count($arr_post) - $per_page*($pager-1);
			$start = ($criteria['blog_id'] == $_GET['blog_id']) && ($pager > 1) ? ($per_page*$pager)-$per_page : 0 ;
			$end = ($criteria['blog_id'] == $_GET['blog_id']) && ($pager > 1) ? ($per_page*$pager) : $per_page ; 
			$pager_pages = ceil((count($arr_post)/$per_page));
			if (($criteria['blog_id'] == $_GET['blog_id']) && ($pager == $pager_pages)) $end = count($arr_post); 
			echo $per_page;
			echo $pager_pages;
			echo $start;
			echo $end;
			...
			echo !empty( $criteria["thumbnail"] ) ? "" : " thumbnail ";
			echo '" data-blogid="' . $blog_id . '">';	
			...
			for($i = $start; $i < $end; $i++) {
				...
				if( !empty( $criteria["thumbnail"] ) ) {
					if( has_post_thumbnail($arr_post[$i]['ID']) ) {
						//	$output .= get_the_post_thumbnail( $arr_post[$i]['ID'], array(100, 100) );
						echo	get_the_post_thumbnail( $arr_post[$i]['ID'], array(100, 100) ); 
						} else {
						//	$output .= '<img class="msp-thumbnail" src="' . plugins_url( "assets/msp_noimg.jpg", __FILE__ ) . '" />';
						echo	'<img class="msp-thumbnail" src="' . plugins_url( "assets/msp_noimg.jpg", __FILE__ ) . '" />';
					}
				}
				echo '<div class="msp-content"><a href="' . get_post_permalink($arr_post[$i]['ID']) . '" title="' . $arr_post[$i]['post_title'] . '">' . $arr_post[$i]['post_title'] . '</a>';
				if( !empty( $criteria["excerpt"] ) ) {
					echo '<p>' . get_the_excerpt() . '</p>';
				}
			 	echo  '<p>' . get_the_author_meta( 'display_name', $arr_post[$i]['post_author'] ) . '</p>';
				echo  '</div></li>';	
				restore_current_blog();
			...
			/* echo '<ul class="pagination">';
				...
				if (($criteria['blog_id'] == $_GET['blog_id'] && ($_GET['pager'] == $i)) 
				|| ($_GET['blog_id'] != $criteria['blog_id'] && !empty($_GET['pager']) && $i == 1) 
				|| (empty($_GET['blog_id']) && empty($_GET['pager']) && $i == 1)) {
				$style = ' active';
				} else {$style = ' ';}
				echo '<li class="paginated_link'.$style.'"><a class="page-numbers" href="/?blog_id='.$blog_id.'&pager='.$i.'">'.$i.'</a></li>';
			...
			$iCurr = (($_GET['blog_id'] != $criteria['blog_id'] && !empty($_GET['pager'])) ||
			(empty($_GET['pager']) || empty($_GET['blog_id'])) ||
			($_GET['blog_id'] == $criteria['blog_id'] && empty($_GET['pager']))) ? 1 : intval($_GET['pager']);
			...
			echo '<ul class="pagination">';
			if ($iCurr != 1)
				echo '<li class="paginated_link"><a class="page-numbers" href="/?blog_id='.$blog_id.'&pager='. ($iCurr-1) .'">&laquo;</a></li>';
			if(($iCurr - $iLeft) > 1 && $iEnd != (1+$iRight+$iLeft))
				echo '<li class="paginated_link"><a class="page-numbers" href="/?blog_id='.$blog_id.'&pager=1">1</a></li>';
			if(($iCurr - $iLeft) > 2)
				echo '<li class="paginated_link"><span class="page-numbers dots">Е</span></li>';
			//центр
			if($iCurr > $iLeft && $iCurr < ($iEnd-$iRight))
			{
				for($i=$iCurr-$iLeft; $i<=$iCurr+$iRight; $i++)
				{
					...
					echo '<li class="paginated_link'.$this->subStyle($criteria, $i).'"><a class="page-numbers" href="/?blog_id='.$blog_id.'&pager='.$i.'">'.($i == $iCurr ? ''.$i.'' : $i).'</a></li>' ; 
				}
			}
			//cлева
			elseif($iCurr<=$iLeft)
			...
			if(($iEnd - $iCurr - $iRight) > 1)
				echo '<li class="paginated_link"><span class="page-numbers dots">Е</span></li>';
			if((($iCurr<=$iLeft) && (($iEnd - $iCurr) > $iRight && $iEnd !=($iCurr+$iRight+$iLeft))) ||
			(($iCurr > $iLeft && $iCurr < ($iEnd-$iRight)) && (($iEnd - $iCurr) > $iRight)))
				echo '<li class="paginated_link"><a class="page-numbers" href="/?blog_id='.$blog_id.'&pager='. $iEnd .'">'. $iEnd .'</a></li>';
			if ($iCurr != $iEnd)
			echo '<li class="paginated_link"><a class="page-numbers" href="/?blog_id='.$blog_id.'&pager='. ($iCurr+1) .'">&raquo;</a></li>';
			echo '</ul>'; 
			//	}
			/* /pag test */		
	...
	class Multisite_Posts {
		...
		function admin_footer_callback() {
			...
			$args 		= array("post_no", "per_page", "category", "blog_id", "custom_query", "excerpt", "thumbnail");
		?>
					...
					<tr class="form-field">
						...
						<td>
							<?php
								...
								if( in_array( $arg, array("post_no", "per_page", "category", "custom_query") ) ) {
	...
	class Multisite_Posts_Widget extends WP_Widget {
		...
		function form( $instance ) {
			...
			$instance 	= shortcode_atts( $this->default, $instance );
			$args 		= array("title", "post_no", "per_page", "category", "blog_id", "custom_query", "excerpt", "thumbnail");
			...
			foreach( $args as $arg ) {
				...
				if( in_array( $arg, array("title", "post_no", "per_page", "custom_query", "category") ) ) {
