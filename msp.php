...
class Multisite_Posts_Core {
	...
	function fetch_msp_posts( $options = false, $blog_id = false, $echo = false, $widget_id = false) {
		...
		$pageNumber = 1;
		if($_REQUEST['blog'] == $blog_id && get_query_var('magic_page_id')) $pageNumber = get_query_var('magic_page_id');
	...
	function msp_bootstrap_paginate_links($max_num_pages, $blog_id, $current, $widget_id = false) {
		...
		$pagination = paginate_links( array(
			...
			'prev_text' => '&laquo;',
			'next_text' => '&raquo;',
			'add_args' => array( 'blog' => $blog_id, 'blog_page' => '%#%')
		) );
		if ( !empty( $pagination ) ) {
			foreach($pagination as $key => $item){
				if(stripos($item, 'href')){
					...
					if(stripos($item,"paged")) {
						...
						$item = substr_replace($item, 'href="'.get_site_url().'/page/'.$page.'/?htype=blog-'.$blog_id.'"' , stripos($item,'href'), stripos($item,'>', stripos($item,'href')) - stripos($item,'href'));
					}
